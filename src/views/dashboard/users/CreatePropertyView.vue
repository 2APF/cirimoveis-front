            ...
            <div class="col-lg-8">
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-3">
                      <div class="form-group">
                        ...
                        <div class="form-check mt-2">
                          <input class="form-check-input" type="checkbox" id="air_conditioning" v-model="form.air_conditioning">
              ...
              <div class="card mt-4">
                ...
                <div class="card-body">
                  ...
                  <div class="image-upload-area mt-4" @dragover.prevent @drop.prevent="onDrop"
...
const validateForm = () => {
  ...
  if (form.description.length < 50) errors.description = 'Mínimo 50 caracteres'
...
const isFormValid = computed(() => {
  return form.name.trim().length >= 10 &&
    form.type &&
    form.price > 0 &&
    form.address.trim() &&
...
<style scoped>
.publish-page { padding-top: 100px; background: linear-gradient(135deg, var(--soft-red) 0%, #f8f9fa 100%); min-height: 100vh; }
.publish-section { padding: 40px 0; }
...
.page-subtitle { font-size: 1.2rem; color: var(--medium-gray); margin: 8px 0 0; }
.card { background: var(--card-bg); border-radius: 16px; box-shadow: 0 6px 20px rgba(0,0,0,0.08); border: 1px solid rgba(0,0,0,0.05); margin-bottom: 24px; }
.card-header { background: linear-gradient(135deg, var(--primary-red), #e53935); color: white; padding: 20px 24px; border-radius: 16px 16px 0 0 !important; }
.card-header h3, .card-header h5 { margin: 0; font-weight: 700; }
.card-body { padding: 24px; }
.form-group { margin-bottom: 20px; }
.form-group label { font-weight: 600; color: var(--dark-charcoal); margin-bottom: 8px; display: block; }
.form-control, .form-select { border: 2px solid #e9ecef; border-radius: 12px; padding: 12px 16px; font-size: 1rem; transition: all 0.3s ease; background: var(--card-bg); }
.form-control:focus, .form-select:focus { border-color: var(--primary-red); box-shadow: 0 0 0 0.2rem rgba(211,47,47,0.15); }
...
.image-upload-area { border: 3px dashed #e9ecef; border-radius: 16px; padding: 40px 20px; text-align: center; cursor: pointer; transition: all 0.3s ease; background: var(--card-bg); }
.image-upload-area:hover { border-color: var(--primary-red); background: rgba(211,47,47,0.02); }
.image-upload-area.drag-over { border-color: var(--primary-red); background: rgba(211,47,47,0.08); }
.images-preview { margin-bottom: 20px; }
.image-item { height: 140px; }
.image-item img { height: 100%; width: 100%; object-fit: cover; border-radius: 12px; }
.btn-remove-image { position: absolute; top: -8px; right: -8px; width: 32px; height: 32px; border-radius: 50%; background: #dc3545; color: white; border: none; display: flex; align-items: center; justify-content: center; }
.sidebar-preview { position: sticky; top: 120px; }
.preview-card { height: fit-content; }
.preview-image img { width: 100%; height: 200px; object-fit: cover; border-radius: 12px; }
.preview-placeholder { height: 200px; display: flex; flex-direction: column; align-items: center; justify-content: center; background: #f8f9fa; border-radius: 12px; border: 2px dashed #e9ecef; }
.preview-price { font-size: 1.5rem; font-weight: 800; color: var(--primary-red); }
.preview-title { color: var(--dark-charcoal); font-weight: 700; margin: 0 0 8px 0; font-size: 1.1rem; }
.preview-location { color: var(--medium-gray); font-size: 0.95rem; margin: 0; }
.preview-features { text-align: left; }
.feature-row { display: flex; justify-content: space-between; font-size: 0.9rem; color: var(--dark-charcoal); margin-bottom: 4px; }
.progress-card { background: linear-gradient(135deg, #f8f9fa, #e9ecef); }
.progress-wrapper { display: flex; flex-direction: column; gap: 12px; margin: 16px 0; }
.progress-item { display: flex; align-items: center; gap: 12px; font-weight: 500; color: var(--medium-gray); font-size: 0.95rem; }
.progress-item.completed { color: var(--primary-red); }
.progress-item i { width: 24px; text-align: center; font-size: 1rem; }
.progress-bar { height: 8px; background: #e9ecef; border-radius: 4px; overflow: hidden; }
.progress-fill { height: 100%; background: linear-gradient(90deg, var(--primary-red), #e53935); border-radius: 4px; transition: width 0.3s ease; }
.progress-text { font-weight: 600; color: var(--primary-red); font-size: 0.9rem; }
.form-actions { display: flex; justify-content: flex-end; gap: 16px; padding-top: 40px; border-top: 1px solid #e9ecef; }
.btn-primary.btn-lg { padding: 16px 32px; font-size: 1.1rem; background: linear-gradient(135deg, var(--primary-red), #e53935); border: none; }
.btn-primary.btn-lg:hover:not(:disabled) { background: linear-gradient(135deg, #b71c1c, #d32f2f); transform: translateY(-2px); }
@media (max-width: 992px) { .page-header { flex-direction: column; text-align: center; } .sidebar-preview { position: static; margin-top: 40px; } .form-actions { flex-direction: column; } .btn-primary.btn-lg { width: 100%; justify-content: center; } }
@media (max-width: 768px) { .page-title { font-size: 2rem; } .card-body { padding: 20px; } .image-item { height: 120px; } }
</style>