...
var detectorInit = function detectorInit() {
  ...
  // Detecção de navegador usando userAgent
  var userAgent = navigator.userAgent;
  
  if (userAgent.indexOf('Opera') !== -1 || userAgent.indexOf('OPR') !== -1) {
    addClass(html, 'opera');
  } else if (userAgent.indexOf('Firefox') !== -1) {
    addClass(html, 'firefox');
  } else if (userAgent.indexOf('Chrome') !== -1) {
    addClass(html, 'chrome');
  } else if (userAgent.indexOf('Safari') !== -1) {
    addClass(html, 'safari');
  } else if (userAgent.indexOf('Edge') !== -1) {
    addClass(html, 'edge');
  } else if (userAgent.indexOf('Trident') !== -1) {
    addClass(html, 'ie'); // Internet Explorer
  }
  
  // Outras detecções
  if (/Mobile|Android|iP(hone|od|ad)/.test(userAgent)) {
    addClass(html, 'mobile');
  }
...
// Chamada para inicializar a detecção
detectorInit();
